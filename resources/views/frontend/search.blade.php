...
@section('metatag')
<title>Tempo.co - Janji Jokowi | Cari Janji {{ucwords(request('key'))}}</title>
				...
				<div class="pilih-inside">
					<div class="area100">
						...
						<form action="{{route('search')}}" method="GET">
							...
							<input id="search" type="text" class="form-area" name="key" required placeholder="cari janji yang ingin ditagih" style="padding-right:52px;" value="{{request('key')}}">
						</form>
						</div>
					</div>
	...
	<div class="container">
		...
		<div id="data">
            ...
            <ul class="box-color">
                ...
                <li>
                 <a href="{{route('front.detail',$item->slug)}}" class="fade">
                     <div class="debox @if ($item->status == 'Terpenuhi') {{'bg-deepblue'}} @elseif ($item->status == 'Dalam Proses') {{'bg-blue'}} @elseif ($item->status == 'Tidak Ada Bukti') {{'bg-yellow'}} @elseif ($item->status == 'Tidak Terverifikasi') {{'bg-grey'}} @else {{'bg-orange'}} @endif">
                         <h3 class="title">{{$item->status}}</h3>
                         <h5>{{strtoupper($item->topik)}}</h5>
                         <h3>{{$item->judul}}</h3>
                     </div>
                 </a>
                </li>
...
$( function () {
    $("#search").onEnter( function() {
		...
		if (key.trim() !== '') {
			...
			var pageurl = "{{ route('search') }}?key="+key;
			$.ajax({
				url:"{{ route('search') }}?key="+key,
				method:"GET",
				data:{_token:_token},
...
$(document).ready(function(){
	$('#periode').change(function(){
		...
		$.ajax({
			url:"{{ route('filter') }}",
			method:"GET",
			data:{_token:_token, periode:periode, topik:topik, key:key},
			success:function(data){
				console.log(data);
